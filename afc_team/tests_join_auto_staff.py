...
class JoinMessageLengthTests(TestCase):
    """Inbox #23 (2026-09-17): a join note longer than the 150-character column reached MySQL,
    which refused it with 1406, and the person saw "An error occurred" and pressed the button
    again (13 times in fourteen seconds on 16 September). The view refuses it first now, names
    the limit, and never echoes an exception to the client."""

    def setUp(self):
        self.client = Client()
        owner = User.objects.create(username="len_owner", email="len_owner@x.com", full_name="o",
                                    role="player", password="x")
        self.team = Team.objects.create(team_name="Length Test", team_tag="LEN", join_settings="open",
                                        team_creator=owner, team_owner=owner, country="NG")
        TeamMembers.objects.create(team=self.team, member=owner, management_role="team_captain")
        asker = User.objects.create(username="len_asker", email="len_asker@x.com", full_name="a",
                                    role="player", password="x")
        SessionToken.objects.create(
            user=asker, token="tok-len-asker",
            expires_at=datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(hours=1))

    def _ask(self, message):
        return self.client.post(REQUEST, data={"team_id": self.team.team_id, "message": message},
                                HTTP_AUTHORIZATION="Bearer tok-len-asker")

    def test_a_151_character_note_is_refused_with_the_limit_named(self):
        resp = self._ask("x" * 151)
        self.assertEqual(resp.status_code, 400, resp.content)
        body = resp.json()
        self.assertEqual(body["code"], "join_message_too_long")
        self.assertEqual((body["max_length"], body["length"]), (150, 151))
        self.assertIn("150", body["message"])
        self.assertFalse(JoinRequest.objects.filter(team=self.team).exists())

    def test_a_150_character_note_goes_through(self):
        resp = self._ask("y" * 150)
        self.assertEqual(resp.status_code, 201, resp.content)
        self.assertEqual(JoinRequest.objects.get(team=self.team).message, "y" * 150)

    def test_an_internal_error_is_logged_and_never_echoed(self):
        from unittest.mock import patch
        with patch("afc_team.views.JoinRequest.objects.create", side_effect=RuntimeError("secret sql detail")), \
                self.assertLogs("afc_team.views", level="ERROR") as logs:
            resp = self._ask("hello")
        self.assertEqual(resp.status_code, 500)
        self.assertEqual(resp.json(), {"message": "An error occurred.", "code": "internal_error"})
        self.assertNotIn("secret sql detail", resp.content.decode())
        self.assertTrue(any("send_join_request failed" in line for line in logs.output))
